...
from ..utils import ensure_wav_16k_mono, ffmpeg_available
...
def _load_audio(whisperx, path: Path):
    def _load_with_soundfile(require_16k: bool) -> Any:
        try:
            ...
            if require_16k and sample_rate != 16000:
                raise RuntimeError(
                    "Audio must be 16kHz when ffmpeg is missing. "
                    "Enable --format_audio or install ffmpeg."
                )
            return audio
        except Exception as exc:
            message = "Failed to load audio. Install soundfile or enable --format_audio."
            if require_16k:
                message = (
                    "Failed to load audio without ffmpeg. "
                    "Install ffmpeg or enable --format_audio."
                )
            raise RuntimeError(message) from exc

    if not ffmpeg_available():
        return _load_with_soundfile(require_16k=True)
